...
if __name__ == "__main__":
    ...
    EMBEDDING_TYPE = "glove"  # Choose between "fasttext", "word2vec", "glove"
    ...
    for name, pooling_fn in pooling_methods.items():
        ...
        model = FFNN(input_dim=dim, hidden_dims=[512, 256], output_dim=len(label_to_idx))
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam(model.parameters(), lr=1e-3)

        train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=10, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"), name=f"{EMBEDDING_TYPE}_{name}", model_type="FFNN")
